...
#ifndef ZEPHYR_INCLUDE_DRIVERS_MFD_AD594X_H_
#define ZEPHYR_INCLUDE_DRIVERS_MFD_AD594X_H_
 ...
 *    @details
 *        It's used to initialize structure @ref sw_matrix_cfg_t
...
typedef struct
{
  ...
  bool BpNotch;             /**< Bypass Notch filter in SINC2+Notch block, so only SINC2 is used. ADCFILTERCON.BIT4 */
  bool BpSinc3;             /**< Bypass SINC3 Module */
  bool Sinc3ClkEnable;      /**< Enable SINC3 clock */
  bool Sinc2NotchClkEnable; /**< Enable SINC2+Notch clock */
 ...
 * Vzero/Vbias switch, controlled by @ref lp_dac_cfg_t LpDacSW
 * Vzero ------PIN
...
typedef struct
{
  ...
  bool LpPaPwrEn;       /**< Enable(bTRUE) or disable(bFALSE) power of PA(potential amplifier) */
  bool LpTiaPwrEn;      /**< Enable(bTRUE) or Disable(bFALSE) power of LPTIA amplifier */
}lp_amp_cfg_t;
...
typedef struct
{
  ...
  wavgen_trapezoid_cfg_t TrapzCfg;   /**< Configure Trapezoid generator */
  wavgen_sin_cfg_t SinCfg;       /**< Configure Sine wave generator */
...
typedef struct
{
  sw_matrix_cfg_t SWMatCfg;  /**< switch matrix configuration. */
  hs_dac_cfg_t HsDacCfg;     /**< HSDAC configuration. */
  wavegen_cfg_t WgCfg;           /**< Waveform generator configuration. */
  hs_tia_cfg_t HsTiaCfg;     /**< HSTIA configuration. */
}hs_loop_cfg_t;
...
typedef struct
{
  lp_dac_cfg_t LpDacCfg;     /**< LPDAC configuration. @note Must select LPDAC0 or LPDAC1 in structure. */
  lp_amp_cfg_t LpAmpCfg;     /**< LPAMP(LPTIA and PA) configuration. @note Must select LPAMP0 or LPAMP1 in structure. */
}lp_loop_cfg_t;
...
typedef struct
{
  adc_base_cfg_t ADCBaseCfg;       /**< ADC base configuration */
  adc_filter_cfg_t ADCFilterCfg;   /**< ADC filter configuration include SINC3/SINC2/Notch/Average(for DFT only) */
  adc_dig_comp_t ADCDigCompCfg;    /**< ADC digital comparator */
  dft_cfg_t DftCfg;               /**< DFT configuration include data source, DFT number and Hanning Window */
  stat_cfg_t StatCfg;             /**< Statistic block */
}dsp_cfg_t;
...
typedef struct
{
  ...
  bool bEnable;         /**< Allow detected pin action to trigger corresponding sequence. */
}ad594x_seq_gpio_trig_cfg_t;
...
typedef struct
{
  ...
  dft_cfg_t DftCfg;         /**< DFT configuration. */
...
typedef struct
{
  ...
  uint32_t LpAmpSel;          /**< Select from LPAMP0 and LPAMP1. LPAMP1 is only available on ADuCM355. */
  bool bWithCtia;         /**< Connect external CTIA or not. */
...
typedef struct
{
  ...
  bool BpNotch;           /**< Bypass notch filter or not. Only used when data type is DATATYPE_DFT and DftSrc is DFTSRC_SINC2NOTCH */
...
}cmplx_impedance_polar_f_t; //Polar
...
}cmplx_impedance_cartesian_f_t; //Cartesian
...
}cmplx_impedance_cartesian_i_t;

/**
 *  freq_params_t - Structure to store optimum filter settings
*/
typedef struct
{
	bool HighPwrMode;
...
typedef int ad594x_err_t;    /**< error number defination */


/**
 * @defgroup mdf_interface_ad594x MFD AD594x interface
...
/**
 * @brief Bidirectional transceive raw data from the chip
 *
 * @param[in] dev Pointer to MFD device
 * @param[in] tx_ptr Pointer to transmit buffer
 * @param[in] rx_ptr Pointer to receive buffer
 * @param[in] len length of data buffer
 *
 * @retval 0 if success
 * @retval negative errno if failure
...
/**
 * @brief Reset the AD594X via SPI command (built-in delay for reset to complete)
 * @param[in] dev Pointer to MFD device
 *
 * @retval 0 if success
 * @retval negative errno if failure
...
/**
 * @brief Reset the AD594X hardware with the reset pin (built-in delay for reset to complete)
 * @param[in] dev Pointer to MFD device
 *
 * @retval 0 if success
 * @retval negative errno if failure
...
void      mfd_add592x_afe_pwr_bw(const struct device *dev, uint32_t AfePwr, uint32_t AfeBw); /* AFE power mode and system bandwidth control */
void      mfd_add592x_ref_cfg(const struct device *dev, afe_ref_cfg_t *pBufCfg);

/* 3. High_Speed_Loop Functions */
void      AD5940_HSLoopCfgS(hs_loop_cfg_t *pHsLoopCfg);
void      AD5940_SWMatrixCfgS(sw_matrix_cfg_t *pSwMatrix);
void      AD5940_HSDacCfgS(hs_dac_cfg_t *pHsDacCfg);
ad594x_err_t AD5940_HSTIACfgS(hs_tia_cfg_t *pHsTiaCfg);
...
void      AD5940_DSPCfgS(dsp_cfg_t *pDSPCfg);
uint32_t  AD5940_ReadAfeResult(uint32_t AfeResultSel);
/* 5.1 ADC Block */
void      AD5940_ADCBaseCfgS(adc_base_cfg_t *pADCInit);
void      AD5940_ADCFilterCfgS(adc_filter_cfg_t *pFiltCfg);
void      AD5940_ADCPowerCtrlS(bool State);
void      AD5940_ADCConvtCtrlS(bool State);
void      AD5940_ADCMuxCfgS(uint32_t ADCMuxP, uint32_t ADCMuxN);
void      AD5940_ADCDigCompCfgS(adc_dig_comp_t *pCompCfg);
void      AD5940_StatisticCfgS(stat_cfg_t *pStatCfg);
void      AD5940_ADCRepeatCfgS(uint32_t Number);
void      AD5940_DFTCfgS(dft_cfg_t *pDftCfg);
/* 5.2 Waveform Generator Block */
void      AD5940_WGCfgS(wavegen_cfg_t *pWGInit);
ad594x_err_t AD5940_WGDACCodeS(uint32_t code); /* Directly write DAC Code */
void      AD5940_WGFreqCtrlS(float SinFreqHz, float WGClock);
uint32_t  AD5940_WGFreqWordCal(float SinFreqHz, float WGClock);
//uint32_t AD5940_WGAmpWordCal(float Amp, bool DacGain, bool ExcitGain);

/* 6. Sequencer_FIFO */
void      AD5940_FIFOCfg(ad594x_fifo_cfg_t *pFifoCfg);
ad594x_err_t AD5940_FIFOGetCfg(ad594x_fifo_cfg_t *pFifoCfg);  /* Read back current configuration */
void      AD5940_FIFOCtrlS(uint32_t FifoSrc, bool FifoEn);   /* Configure FIFO data source. And disable/enable it.*/
void      AD5940_FIFOThrshSet(uint32_t FIFOThresh);
uint32_t  AD5940_FIFOGetCnt(void);     /* Get current FIFO count */
void      AD5940_SEQCfg(ad594x_seq_cfg_t *pSeqCfg);
ad594x_err_t AD5940_SEQGetCfg(ad594x_seq_cfg_t *pSeqCfg);    /* Read back current configuration */
void      AD5940_SEQCtrlS(bool SeqEn);
...
void      AD5940_SEQInfoCfg(ad594x_seq_info_t *pSeq);
ad594x_err_t AD5940_SEQInfoGet(uint32_t SeqId, ad594x_seq_info_t *pSeqInfo);
...
ad594x_err_t AD5940_SEQGpioTrigCfg(ad594x_seq_gpio_trig_cfg_t *pSeqGpioTrigCfg);
void      AD5940_WUPTCfg(ad594x_wk_tim_cfg_t *pWuptCfg);
void      AD5940_WUPTCtrl(bool Enable);  /* Enable or disable Wakeup timer */
ad594x_err_t AD5940_WUPTTime(uint32_t SeqId, uint32_t SleepTime, uint32_t WakeupTime);

/* 7. MISC_Block */
/* 7.1 Clock system */
void      AD5940_CLKCfg(ad594x_clk_cfg_t *pClkCfg);
...
/* 7.2 AFE Interrupt */
void      AD5940_INTCCfg(uint32_t AfeIntcSel, uint32_t AFEIntSrc, bool State);
uint32_t  AD5940_INTCGetCfg(uint32_t AfeIntcSel);
void      AD5940_INTCClrFlag(uint32_t AfeIntSrcSel);
bool  AD5940_INTCTestFlag(uint32_t AfeIntcSel, uint32_t AfeIntSrcSel); /* Check if selected interrupt happened */
uint32_t  AD5940_INTCGetFlag(uint32_t AfeIntcSel); /* Get current INTC interrupt flag */
/* 7.3 GPIO */
void      AD5940_AGPIOCfg(ad594x_gpio_cfg_t *pAgpioCfg);
...
/* 7.4 LPMODE */
ad594x_err_t 	  mfd_ad594x_lpmode_toggle(const struct device *dev, bool LPModeEn); /* Enable LP mode or disable it. */
...
/* 7.5 Power */
void      mfd_ad594x_sleepkey_ctrl(const struct device *dev, uint32_t slp_key); /* enter the correct key to allow AFE to enter sleep mode */
void      mfd_ad594x_enter_sleep(const struct device *dev);      /* Put AFE to hibernate/sleep mode and keep LP loop as the default settings. */
void      mfd_ad594x_hibernate(const struct device *dev);    /* Unlock the key, turn off LP loop and enter sleep/hibernate mode  */
uint32_t  mfd_ad594x_wake_up(const struct device *dev, int32_t TryCount));   /* Try to wakeup AFE by read register */
uint32_t  mfd_ad594x_get_adiid(const struct device *dev);   /* Read ADIID */
uint32_t  mfd_ad594x_get_chip_id(const struct device *dev);  /* Read Chip ID */
/* Calibration functions */
/* 8. Calibration */
ad594x_err_t AD5940_ADCPGACal(adc_pga_cal_t *ADCPGACal);
ad594x_err_t AD5940_LPDACCal(lp_dac_cal_t *pCalCfg, lp_dac_params_t *pResult);
ad594x_err_t AD5940_LPTIAOffsetCal(lptia_offset_cal_t *pLPTIAOffsetCal);
ad594x_err_t AD5940_HSRtiaCal(hs_rtia_cal_t *pCalCfg, void *pResult);
ad594x_err_t AD5940_HSDACCal(hs_dac_cal_t *pCalCfg);
ad594x_err_t AD5940_LPRtiaCal(lp_rtia_cal_t *pCalCfg, void *pResult);
ad594x_err_t AD5940_LFOSCMeasure(lf_osc_measure_t *pCfg, float *pFreq);
...
void      AD5940_SEQGenCtrl(bool bFlag);  /* Enable or disable sequence generator */
void      AD5940_SEQGenInsert(uint32_t CmdWord); /* Manually insert a sequence command */
ad594x_err_t AD5940_SEQGenFetchSeq(const uint32_t **ppSeqCmd, uint32_t *pSeqCount);  /* Fetch generated sequence and start a new sequence */
void      AD5940_ClksCalculate(clks_cal_info_t *pFilterInfo, uint32_t *pClocks);
uint32_t  AD5940_SEQCycleTime(void);
void      AD5940_SweepNext(soft_sweep_cfg_t *pSweepCfg, float *pNextFreq);
...
bool  AD5940_Notch50HzAvailable(adc_filter_cfg_t *pFilterInfo, uint8_t *dl);
bool  AD5940_Notch60HzAvailable(adc_filter_cfg_t *pFilterInfo, uint8_t *dl);
cmplx_impedance_cartesian_f_t AD5940_ComplexDivFloat(cmplx_impedance_cartesian_f_t *a, cmplx_impedance_cartesian_f_t *b);
cmplx_impedance_cartesian_f_t AD5940_ComplexMulFloat(cmplx_impedance_cartesian_f_t *a, cmplx_impedance_cartesian_f_t *b);
cmplx_impedance_cartesian_f_t AD5940_ComplexAddFloat(cmplx_impedance_cartesian_f_t *a, cmplx_impedance_cartesian_f_t *b);
cmplx_impedance_cartesian_f_t AD5940_ComplexSubFloat(cmplx_impedance_cartesian_f_t *a, cmplx_impedance_cartesian_f_t *b);

cmplx_impedance_cartesian_f_t AD5940_ComplexDivInt(cmplx_impedance_cartesian_i_t *a, cmplx_impedance_cartesian_i_t *b);
cmplx_impedance_cartesian_f_t AD5940_ComplexMulInt(cmplx_impedance_cartesian_i_t *a, cmplx_impedance_cartesian_i_t *b);
float     AD5940_ComplexMag(cmplx_impedance_cartesian_f_t *a);
float     AD5940_ComplexPhase(cmplx_impedance_cartesian_f_t *a);
freq_params_t AD5940_GetFreqParameters(float freq);
/**
...
#endif /* ZEPHYR_INCLUDE_DRIVERS_MFD_AD594X_H_ */
